...
# and, for each cache, train both a linear and an mlp head      = 12 head runs.
#
# The 6 cache configs are spread round-robin across the 3 GPUs (2 per GPU).
# Each GPU processes its configs sequentially (embed → train linear → train mlp),
# and the three GPU lanes run concurrently. Caches use the pure-PyTorch ("torch")
# attention backend.
#
...
#
#   bash train_pipeline/run_window_sweep.sh
#
set -uo pipefail
...
# ── One full config: embed the cache, then train every head on it ─────────────
...
echo "Sweep: ${#configs[@]} caches × ${#HEADS[@]} heads on 3 GPUs (backend=$BACKEND)"
echo "  gpu0: ${gpu0[*]}"
echo "  gpu1: ${gpu1[*]}"
echo "  gpu2: ${gpu2[*]}"
...
echo "[$(date +%H:%M:%S)] Sweep complete. Trained heads under $HEAD_DIR/"
